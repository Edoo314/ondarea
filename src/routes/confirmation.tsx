...
export const Route = createFileRoute("/confirmation")({
  head: () => ({
    meta: [
      { title: "Confirmation — Ondarea" },
      { name: "description", content: "Votre demande est prête à être envoyée. Vérifiez le récapitulatif et ouvrez votre client e-mail." },
      { property: "og:title", content: "Ondarea — Confirmation de votre demande" },
      { property: "og:description", content: "Récapitulatif de votre demande de recueil de mémoire." },
    ],
  }),
...
function ConfirmationPage() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("ondarea-contact");
      if (!raw) {
  ...
  const mailtoHref = `mailto:bonjour@ondarea.fr?subject=${encodeURIComponent(mailtoSubject)}&body=${encodeURIComponent(mailtoBody)}`;

  function sendMail() {
    window.location.href = mailtoHref;
    setSent(true);
    try {
      sessionStorage.removeItem("ondarea-contact");
    } catch {
      // ignore
    }
  }
    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <main className="mx-auto max-w-3xl px-6 py-20 md:py-28">
        ...
        <p className="mt-6 max-w-xl text-muted-foreground leading-relaxed">
          ...
          envoyer à Ondarea. Votre client e-mail s'ouvrira avec le message
          pré-rempli.
        </p>
        ...
        <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center">
          {sent ? (
            <div className="rounded-sm border border-ink/15 bg-paper px-6 py-4 text-sm text-muted-foreground">
              ...
              <a href="mailto:bonjour@ondarea.fr" className="text-ink underline">bonjour@ondarea.fr</a>.
            </div>
          ) : (
            <>
              <button
